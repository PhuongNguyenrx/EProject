...
public class ImagePanel : MonoBehaviour
{
    ...
    [SerializeField] private float minZoomScale = 0.5f; // Minimum scale allowed for zooming
    [SerializeField] private float maxZoomScale = 2f; // Maximum scale allowed for zooming
    ...
    private void HandleTouchInput()
    {
        ...
        else if (touch.phase == TouchPhase.Moved && isPanActive)
        {
            ...
            panelRectTransform.anchoredPosition += delta * panSpeed;
            touchStart = touch.position;
        }
        else if (touch.phase == TouchPhase.Ended)
        {
            if(delta.magnitude<=2) gameObject.SetActive(false);
            isPanActive = false;
            delta = Vector2.zero;
            ClampPanPosition();
        }
